...
fn search_from_node(path_so_far: &Vec<String>, has_revisit: bool, edges: &HashMap<String, HashSet<String>>) -> Vec<Vec<String>> {
    let mut paths = vec![];
    ...
    if let Some(edges_from_node) = edges.get(&node) {
        for to in edges_from_node {
            ...
            if node_is_revisitable(to) || !path_so_far.contains(to) {
                let mut path_with_this_edge = path_so_far.to_vec();
                path_with_this_edge.push(to.clone());

                for path in search_from_node(&path_with_this_edge, has_revisit, edges).iter() {
            ...
            if !has_revisit {
                ...
                for path in search_from_node(&path_with_this_edge, true, edges).iter() {
                    paths.push(path.clone());
                }
            }
        }
...
fn main() {
    ...
    let paths = search_from_node(&vec![String::from("start")], false, &edges);
