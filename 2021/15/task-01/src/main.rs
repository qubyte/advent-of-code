...
fn main() {
    let dangers: Vec<Vec<usize>> = stdin()
        .lock()
        .lines()
    ...
    while vertices.len() != 0 {
        let v_c = vertices.clone();
        let vertex = v_c.iter().min_by(|a_coord, b_coord| {
        ...
        if *i > 0 {
            let d = distances.get(&(*i, *j)).unwrap() + dangers[*j][*i - 1];
        ...
        if *j > 0 {
            let d = distances.get(&(*i, *j)).unwrap() + dangers[*j - 1][*i];
